...
import logging
_logger = logging.getLogger('odoo.addons.jasper_connector.jasperlib')
...
class Jasper(object):
    ...
    def send(self, soapQuery=''):
        _logger.debug('SOAP query: %s', soapQuery)
        try:
    ...
    def create_request(self, operation='list', wsType='', uri='/', name='',
        ...
        _logger.debug('The result returned on create_request: %s', res)
